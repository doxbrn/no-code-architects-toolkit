...
def stream_upload_to_s3(file_url, custom_filename=None, make_public=False):
    ...
    try:
        ...
        logger.info(
            f"Starting multipart upload for {filename} to bucket "
            f"{bucket_name}"
        )
...
def upload_fileobj_to_s3(fileobj, filename, make_public=False):
    """
    Faz upload de um file-like object (por exemplo, de um upload Flask) para o S3.
    Args:
        fileobj: file-like object (por exemplo, request.files['file'])
        filename: nome do arquivo no S3
        make_public: se True, arquivo será público
    Returns:
        dict: informações sobre o upload
    """
    try:
        bucket_name = os.environ.get('S3_BUCKET_NAME', '')
        endpoint_url = os.getenv('S3_ENDPOINT_URL')
        ...
        logger.error(
            f"Erro ao fazer upload do arquivo binário para o S3: {e}"
        )
        raise