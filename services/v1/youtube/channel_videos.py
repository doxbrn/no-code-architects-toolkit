...
# Fallback para a chave fixa apenas se a variável de ambiente não estiver
# disponível
...
def parse_iso8601_duration(duration_str: str) -> int:
    ...
    try:
        ...
        logger.warning(
            f"Could not parse duration: {duration_str}. Returning 0."
        )
        return 0
...
def get_videos_by_channel_id(
    ...
    **kwargs  # Aceita argumentos extras como order_by e os ignora
    ...
    """
    Obtém informações de vídeos de um canal do YouTube usando a playlist de
    uploads.
    ...
    Args:
        ...
        **kwargs: Permite que argumentos extras (como order_by da rota)
                  sejam passados mas não usados.

    Returns:
        Um dicionário contendo informações do canal e seus vídeos formatados
        para Airtable.
    """
    # order_by = kwargs.get('order_by', 'viewCount') # Se precisasse usar
    logger.info(
        f"Chamada a get_videos_by_channel_id para channel_id: {channel_id}, "
        f"max_results: {max_results}. Argumentos extras ignorados: {kwargs}"
    )
    try:
        ...
        uploads_playlist_id = channel_info.get('contentDetails', {})\
            .get('relatedPlaylists', {}).get('uploads')

        if not uploads_playlist_id:
            logger.warning(
                f"Playlist de uploads não encontrada para o canal: {channel_id}"
            )
            return {
        ...
        logger.info(
            f"Playlist de uploads para {channel_id}: "
        ...
        # Limit to 10 pages (500 videos) as a safeguard
        max_pages_to_query = min(max_pages_to_query, 10)

        logger.info(
            f"Iniciando busca de vídeos da playlist de uploads "
            f"({uploads_playlist_id}) para o canal {channel_id}. "
            f"Max results: {max_results}, Max pages: {max_pages_to_query}"
        )

        while (total_fetched_ids_count < max_results and
               page_count < max_pages_to_query):
            ...
            playlist_items_response = youtube.playlistItems().list(
                part='contentDetails',  # We only need contentDetails.videoId
            ...
            current_page_video_ids = [
                ...
                if (item.get('contentDetails') and
                    item['contentDetails'].get('videoId'))
            ]

            if not current_page_video_ids:
                s_reason = (
                    f"API returned no video IDs from playlistItems on page "
                    f"{page_count}."
                )
                logger.info(
                    f"Nenhum ID de vídeo na página {page_count} da playlist. "
                    f"{s_reason}"
                )
            ...
            logger.debug(
                ...
                f"página {page_count} da playlist. Total de IDs coletados: "
                f"{total_fetched_ids_count}."
            )

            next_page_token = playlist_items_response.get('nextPageToken')
            if not next_page_token:
                s_reason = (
                    f"API no next page token for playlistItems after page "
                    f"{page_count}."
                )
                logger.info(
                    f"Não há mais páginas na playlist após {page_count}. "
                    f"{s_reason}"
                )
                channel_data_response["pagination_stop_reason"] = s_reason
                break
            if total_fetched_ids_count >= max_results:
                s_reason = (
                    f"max_results ({max_results}) reached during playlist "
                    f"ID fetching."
                )
                logger.info(
                    f"Limite de {max_results} vídeos atingido. {s_reason}"
                )
                channel_data_response["pagination_stop_reason"] = s_reason
                break
        ...
        if not channel_data_response["pagination_stop_reason"] and \
           total_fetched_ids_count < max_results:
            channel_data_response["pagination_stop_reason"] = (
                "Loop finished (max pages reached or API limit for "
                "playlistItems)."
            )
        ...
        if collected_video_ids:
            ...
            for i in range(0, len(collected_video_ids), 50):
                ...
                logger.debug(
                    f"Buscando detalhes para lote de {len(batch_ids)} IDs de "
                    f"vídeo."
                )
                ...
                videos_details_list.extend(
                    video_details_response.get('items', [])
                )
        ...
        for item in videos_details_list:
            ...
            video_data_for_airtable = {
                ...
                # Convertendo para string 'true'/'false'
                "Has_Caption":
                    str(content_details.get('caption', 'false')).lower(),
                "Is_Licensed_Content":
                    str(content_details.get('licensedContent', False)).lower(),
                ...
                "Are_Public_Stats_Viewable":
                    str(status.get('publicStatsViewable', False)).lower(),
        ...
        channel_data_response['total_videos_fetched'] = \
            len(airtable_formatted_videos)
        ...
        err_content = (e.response.get('content', b'').decode()
                       if e.response else str(e))
        logger.error(
            f"Erro na API do YouTube para o canal {channel_id}: {err_content}"
        )
        return {
        ...
        logger.error(
            f"Erro geral ao processar dados para o canal {channel_id}: "
            f"{str(e)}",
            exc_info=True
        )
        return {
